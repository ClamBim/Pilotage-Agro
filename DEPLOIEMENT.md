...
| Fichier | Rôle |
|---|---|
| `index.html` | l'application complète (un seul fichier, ~1,4 Mo, bibliothèque cartographique Leaflet incluse), configurée pour le projet Supabase `aekyxbvlzpgarqhnqnks` |
...
| **AMO** | tous les sites, taux (estimation et marché), base du tableau de bord, objectif, scénarios et contrôles ; ne gère pas les comptes |
...
## 6. Documents Topo & VRD — le panneau « Documents »

Dans l'onglet **Topo & VRD**, un panneau rétractable occupe le bord droit de la page : c'est là que se déposent, se classent et s'ouvrent les documents des secteurs (plans de récolement, levés, maquettes VRD, photos, dessins CAO, fichiers Office…). Le bouton **▶** le replie en un rail vertical (« Documents · 118 ») qui reste cliquable et accepte lui aussi les fichiers glissés ; l'état replié/déplié est mémorisé dans le navigateur. Sur un écran de moins de 1 500 px, le panneau s'ouvre replié.

**Déposer.** « Fichiers… » choisit un ou plusieurs fichiers, « Dossier… » un dossier entier (sous-dossiers compris) ; on peut aussi glisser des fichiers ou un dossier sur la zone en pointillés, sur le rail, ou **directement sur la carte d'un secteur** de la page, qui les classe dans ce secteur. Tous les formats sont acceptés, jusqu'à **200 Mo par fichier** ; les fichiers vides, trop gros ou déjà présents (même nom, même taille) sont ignorés et comptés dans le message de fin. Les fichiers partent trois par trois dans le bucket `documents` du projet Supabase (chemin `topo/<secteur>/<id>-<nom>`), leur fiche (nom, taille, type, secteur, dossier d'origine, commentaire, date) dans la table partagée : **tous les utilisateurs de la page voient et ouvrent les mêmes documents**. Une barre de progression suit le dépôt ; « Arrêter » interrompt après les fichiers en cours.

**Classer et retrouver.** La liste déroulante « Classer dans » choisit le secteur des fichiers ajoutés (« Non classé » par défaut, ou le secteur filtré). Le champ de recherche porte sur le nom, le dossier d'origine, le commentaire et le secteur, sans tenir compte des accents. Les puces filtrent par secteur et par type ; les listes déroulantes trient (récents, nom, taille, type) et regroupent : **par secteur**, **par bâtiment** (le numéro à quatre chiffres lu dans le nom du fichier — `Bat_0060_Niveau_2.pdf` rejoint « BAT 0060 - BATIMENT ENSEIGNEMENT »), par type, ou sans groupe. Chaque groupe se replie d'un clic sur son titre.

**Fiche du document.** Un clic sur une ligne ouvre sa fiche en bas du panneau : secteur et commentaire modifiables, boutons *Ouvrir*, *Télécharger*, *Retirer*. Les cases à cocher permettent une **sélection multiple** (« Tout sélectionner » prend les lignes affichées) : la barre bleue qui apparaît classe d'un coup les documents cochés dans un secteur, ou les retire.

**Visionneuse.** L'œil, le double clic ou <kbd>Entrée</kbd> ouvrent le document plein écran : PDF et fichiers texte dans un cadre de lecture, images à l'écran (clic : taille réelle), maquettes IFC en 3D avec le nombre d'éléments et de triangles, le filtrage par niveau et par famille et le clic sur un élément pour ses informations (*Ajuster*, *Vue de dessus*, *Face*, *Image* pour une capture PNG). Les autres formats (DWG, Office, archives…) n'ont pas d'aperçu dans le navigateur : la visionneuse propose le téléchargement. Les boutons **‹ ›** et les flèches du clavier passent au document précédent ou suivant dans l'ordre de la liste ; <kbd>Échap</kbd> ferme. Au-delà de 80 Mo, une maquette IFC reste téléchargeable mais n'est pas affichée en 3D (limite du rendu en navigateur) ; même règle pour les images au-delà de 40 Mo.

- *Retirer* supprime la fiche pour tout le monde et le fichier du stockage.
- Un compte **lecteur** voit la liste, ouvre et télécharge, mais ne peut ni déposer, ni classer, ni retirer (boutons grisés, règles du stockage côté serveur).
...
- Les fiches anciennes ou incomplètes (type ou taille absents) restent affichées ; si le fichier n'est plus dans le stockage, la visionneuse l'indique clairement.
...
## 9. Taux : estimation AMO, puis taux du marché

L'onglet **Taux** porte deux grilles de taux unitaires et un réglage qui dit laquelle valorise le tableau de bord.

**Neuf prestations budgétées** — les huit prestations de modélisation du classeur (collecte, Archi LOD100/LOD300, MEP-1/2/3, visionneuses) et la **numérisation 3D**, prestation à part entière dont le montant est le prix scan de chaque fiche (type de scan × taux BIM-1 ou BIM-2, ou montant saisi). Le budget d'un bâtiment, les totaux par site, le bandeau, Suivi budget (une ligne « Numérisation 3D » par site, avec commande et facture), les scénarios (la numérisation suit le champ « Numérisation ») et les exports (colonne « Scan 3D ») la comptent tous.

**Taux de l'estimation** — la grille de l'AMO (forfait par bâtiment, € par m² de SDP, ou € par m² et par niveau ; forfait réduit pour les petits bâtiments). Les taux se saisissent avec une virgule ou un point (« 0,95 » ou « 0.95 ») ; les colonnes *Estimation aux taux* et *Écart*, l'indicateur « Estimation recalculée aux taux » et les boutons « Appliquer » se mettent à jour pendant la frappe. Modifier un taux ne change aucun montant de l'estimation tant que le recalcul n'est pas appliqué : « Appliquer les taux à l'estimation » par site ou pour tous, ou bâtiment par bâtiment dans sa fiche (⟳). Seules les prestations déjà chiffrées (> 0 €) sont recalculées ; les montants saisis à la main sont remplacés. « Taux constaté » est la moyenne observée dans les montants actuels, « Taux par défaut » rétablit les taux de référence de la base.

**Taux du marché** — après l'appel d'offres, saisir dans cette grille les prix du titulaire (mode, taux, forfait petit bâtiment), ainsi que la référence du marché, le titulaire et la date de notification dans l'encadré « Base du tableau de bord ». Il n'y a rien à appliquer : les montants du marché se calculent en direct, prestation par prestation, avec les quantités des fiches (SDP, niveaux, forfaits) et sont comparés à l'estimation (écart en euros et en %, par prestation et par site). Un taux vide reprend l'estimation telle quelle pour cette prestation ; un taux à 0 est un vrai 0 (prestation incluse dans un autre prix). « Reprendre les taux de l'estimation » pré-remplit la grille pour n'ajuster que ce qui change.

**Base du tableau de bord** — le sélecteur **Estimation AMO / Marché** (réservé aux rôles AMO et administrateur, commun à tous les utilisateurs) choisit les montants affichés dans toute l'application : bandeau, tableau de bord, sites, fiches, Suivi budget, contrôles et exports. En base **Marché** :

- le bandeau indique « Marché bâtiments » et « Marché Topo & VRD » avec l'estimation en rappel, et le tableau de bord affiche un bandeau (titulaire, marché, écart) ;
- dans **Suivi budget**, une colonne *Estimation AMO* précède le *Budget marché* ; pour les secteurs Topo & VRD, l'estimation reste modifiable et le budget marché = surface cadastrale × taux Topo du marché (secteurs à 0 € exclus) ;
- dans chaque **fiche**, « Budget marché » avec l'estimation en rappel, un tableau estimation / marché / écart par prestation, et les données d'entrée qui restent celles de l'estimation (le montant marché est rappelé à côté de chaque montant saisi) ;
- les **scénarios** sont valorisés aux montants du marché ; leur intégration au suivi écrit toujours dans l'estimation (données d'entrée) ;
- l'**export Excel** ajoute les colonnes marché, estimation et écart, et la feuille Taux reprend les deux grilles ; l'export CSV ajoute le total estimation.

En base **Marché**, modifier et appliquer les taux de l'estimation met à jour l'estimation (colonnes « Estimation », rappels « estim. »), mais pas les montants du tableau de bord, qui suivent les taux du marché — un encadré le rappelle au-dessus de la grille. Revenir sur **Estimation AMO** ne perd rien : les taux du marché sont conservés. « Réinitialiser le suivi » (onglet Données) conserve les deux grilles et la base choisie ; « Réinitialiser les données d'entrée » efface les taux du marché et revient à l'estimation. Les réglages sont enregistrés dans une seule ligne `marche` de la table (réglage global, lisible par tout compte actif).

## 10. Reprendre les saisies existantes
...
## 12. Changer de projet ou de table
