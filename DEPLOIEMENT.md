...
| Fichier | Rôle |
|---|---|
| `index.html` | l'application complète (un seul fichier, ~1,37 Mo, bibliothèque cartographique Leaflet incluse), configurée pour le projet Supabase `aekyxbvlzpgarqhnqnks` |
| `plans-la-gaillarde-1sur4.zip` … `4sur4.zip` | le dossier `plans/` en quatre archives (limite d'envoi de 30 Mo) : **décompressez les quatre au même endroit**, à côté de `index.html` — elles se complètent dans la même arborescence |
...
## 7. Plans PDF des bâtiments — onglet « Plans »

Les plans du campus de La Gaillarde sont **livrés avec la page** : le dossier `plans/` se dépose à côté de `index.html`, dans le même dépôt GitHub. Rien à téléverser, rien à configurer — l'application connaît déjà la liste (elle est inscrite dans `index.html` au moment de la construction) et lit les fichiers directement.

```
votre-depot/
├── index.html
└── plans/
    ├── 0010/Bat_0010_Niveau_0.pdf      ← les plans, un dossier par bâtiment
    ├── 0120/…
    └── _vig/0010/Bat_0010_Niveau_0.jpg ← les vignettes (première page, 380 px)
```

**Contenu livré :** 134 PDF, 64 bâtiments couverts, 73 Mo dossier compris (dont un plan scanné de 19 Mo). Trois bâtiments n'ont pas de plan dans les dossiers transmis : **BAT 0131, BAT 0244, BAT 0311**.

**L'onglet « Plans »** (menu *Livrables*) réunit le tout :

- à gauche, la liste des bâtiments — ceux qui ont des plans d'abord, avec le nombre de niveaux ; filtre par site et recherche par nom ou numéro ;
- en haut, une **vignette par niveau** (l'image de la première page) : on reconnaît le plan avant de l'ouvrir. Les niveaux sont triés (`Niveau 0`, `Niveau 1`, mezzanine juste après, `Toiture` à la fin), d'après le nom du fichier ;
- au centre, le **plan en grand**, lu par la visionneuse PDF du navigateur : zoom, recherche, impression. Le bouton **⛶ Plein écran** donne toute la fenêtre au plan (Échap pour revenir), **⤓ Télécharger** enregistre le fichier.

Le cadre du plan est conservé d'un affichage à l'autre : changer de bâtiment ou d'onglet ne recharge pas le PDF.

**Dans la fiche d'un bâtiment**, un bloc **Plans** montre les vignettes des niveaux ; un clic ouvre l'onglet Plans sur ce plan précis.

**Ajouter des plans.** Le bouton « Ajouter des plans… » de l'onglet, ou un glisser-déposer sur la zone du plan (ou sur la fiche du bâtiment), dépose des PDF supplémentaires dans le stockage du projet Supabase — utile pour un plan corrigé ou un bâtiment non couvert, sans reconstruire la page. Ces plans-là suivent le périmètre du bâtiment : clé `docs/<bâtiment>.<id>`, chemin `plans/<bâtiment>/…` dans le bucket, dépôt et lecture refusés hors périmètre par les policies (`stb_obj_scope`). Le rattachement est automatique : le **code à quatre chiffres** du nom de fichier désigne le bâtiment (`Bât 0120_Niveau 1.pdf` → BAT 0120), un fichier portant plusieurs codes est rattaché à chacun (`Bât 0125_0126_0127_Niveau 0.pdf`, stocké une seule fois), et un fichier déjà présent sous le même nom est ignoré.

> Ces règles de périmètre sont posées par la dernière version de `supabase-setup.sql` : **rejouez le script** dans l'éditeur SQL du projet. Il est rejouable sans risque.

**Mettre à jour les plans livrés.** Remplacer un fichier dans `plans/` suffit pour que la page affiche le nouveau. En revanche, **ajouter ou retirer un fichier** demande de régénérer la liste embarquée dans `index.html` : c'est fait à la construction de la page, dites-le moi et je vous renvoie le couple `index.html` + `plans/`.

Un bâtiment sans plan est signalé dans l'onglet *Contrôles*. Dans la version hébergée sur claude.ai, l'onglet existe mais reste vide : le dossier `plans/` n'accompagne pas cette page (73 Mo, très au-delà de ce qu'un artifact peut porter).


## 8. Contrôle qualité des livrables IFC et verrouillage de l'onglet
...
## 9. Scénarios et intégration au suivi
...
- Le mot de passe de l'onglet Contrôles (§8) fait maintenant double emploi avec les rôles : il reste disponible, mais c'est le rôle qui protège réellement. Vous pouvez retirer cette protection sans rien perdre.
